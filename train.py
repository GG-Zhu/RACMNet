...
parser.add_argument('--data_path', default='', type=str,
                    help='dataset root; use default SYSU-MM01 or RegDB path when empty')
...
if dataset == 'sysu':
    data_path = args.data_path or '../datasets/SYSU-MM01/'
    n_class = 395
    test_mode = [1, 2]
elif dataset =='regdb':
    data_path = args.data_path or '../datasets/RegDB/'
    n_class = 206
    test_mode = [2, 1]
...
if dataset == 'sysu':
    ...
    for trial in range(10):
        ...
        model_path = checkpoint_path + 'regdb_agw_p4_n8_lr_0.1_seed_0_trial_{}_best.t'.format(test_trial)
        if os.path.isfile(model_path):
            print('==> loading checkpoint {}'.format(args.resume))
            checkpoint = torch.load(model_path)
            net.load_state_dict(checkpoint['net'])
