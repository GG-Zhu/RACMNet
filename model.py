...
class CrossModalityCenterConsistentTrainingLoss(nn.Module):
    """Cross-Modality Center-Consistent Training objective (CMCT)."""

    def __init__(self, k_size=4, margin=0.7, lambda_cm=0.3):
        super(CrossModalityCenterConsistentTrainingLoss, self).__init__()
        self.margin = margin
        self.k_size = k_size
    ...
    def forward(self, inputs, targets):
        ...
        centers = torch.stack([
            inputs[targets == targets[i]].mean(0)
            for i in range(n)
        ])

                                                      
        dist_pc = ((inputs - centers) ** 2).sum(1).sqrt()

                                                             
        dist = comp_dist(centers, centers)
        ...
        for i in range(n):
            dist_an.append(
                (self.margin - dist[i][mask[i] == 0]).clamp(min=0.0).mean()
            )
        dist_an = torch.stack(dist_an)

        half = n // 2
        inputs_rgb = inputs[:half]
        inputs_ir  = inputs[half:]
        targets_rgb = targets[:half]
        targets_ir  = targets[half:]
        ...
        for i in range(half):
            centers_rgb.append(
                inputs_rgb[targets_rgb == targets_rgb[i]].mean(0)
            )
            centers_ir.append(
                inputs_ir[targets_ir == targets_ir[i]].mean(0)
            )

        centers_rgb = torch.stack(centers_rgb)
        centers_ir  = torch.stack(centers_ir)

        loss_cm = ((centers_rgb - centers_ir) ** 2).sum(1).sqrt().mean()

                                   
        loss = dist_pc.mean() + dist_an.mean() + self.lambda_cm * loss_cm
...
class ReliabilityCompactnessLoss(nn.Module):
     """Reliability Compactness Constraint (RCC)."""

     def __init__(self):
         super(ReliabilityCompactnessLoss, self).__init__()

     def forward(self, s_mask):
         loss = torch.mean(torch.abs(s_mask))
         return loss
...
class QuarCenterTripletLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        centers_dualIR1 = torch.stack(centers_dualIR1)    
        centers_dualIR2 = torch.stack(centers_dualIR2)       
        centers_dualRGB1 = torch.stack(centers_dualRGB1)      
        centers_dualRGB2 = torch.stack(centers_dualRGB2)     
        
        dist_pc_dual = (inputs - centers_dual)**2                       
